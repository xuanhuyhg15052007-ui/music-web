...
const PlaylistManager = (() => {
  ...
  function bindCreateModal() {
    ...
    const btnOpen     = document.querySelectorAll('[data-action="create-playlist"]');
    ...
    btnOpen.forEach(btn => btn.addEventListener('click', () => openModal(overlay)));
    ...
    form?.addEventListener('submit', async (e) => {
      ...
      try {
        const newPl = await window.API.playlists.create({ name, description: desc });
        myPlaylists.push(newPl);
        renderSidebarPlaylists(myPlaylists, document.querySelector('.sidebar-playlist-list'));
        closeModal(overlay);
        nameInput.value = '';
  ...
  return {
    ...
    playSongAt,
    removeSong,
    get playlists() { return myPlaylists; },
  };
