...
const Player = (() => {
  ...
  function getRandomIndex(length, excludeIndex) {
    ...
    let rand;
    do { rand = Math.floor(Math.random() * length); }
  ...
  function updateVolumeUI() {
    ...
    const pct = vol * 100;
    el.volumeSlider.style.background =
      `linear-gradient(to right, var(--color-text-primary) ${pct}%, var(--color-border-light) ${pct}%)`;
  ...
  function updateNowPlayingUI(song) {
    ...
    if (el.trackArtist) el.trackArtist.textContent = song.artist || 'Nghệ sĩ chưa xác định';
  ...
  function handleKeyboard(e) {
    ...
    switch (e.code) {
      ...
      case 'ArrowRight':
        ...
        else if (audio.src) { audio.currentTime = Math.min(audio.currentTime + 10, audio.duration); }
        break;
      case 'ArrowLeft':
        if (e.ctrlKey) { e.preventDefault(); playPrev(); }
        else if (audio.src) { audio.currentTime = Math.max(audio.currentTime - 10, 0); }
        break;
      case 'KeyM':
        toggleMute();
        break;
      case 'ArrowUp':
        if (e.ctrlKey) { e.preventDefault(); setVolume(state.volume + 0.1); }
        break;
      case 'ArrowDown':
        if (e.ctrlKey) { e.preventDefault(); setVolume(state.volume - 0.1); }
        break;
