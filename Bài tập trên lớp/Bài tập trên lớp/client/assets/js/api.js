...
// ============================================
const _SHARED_SONGS = [
  { _id: "s1", id: "s1", title: "Making My Way", artist: "Sơn Tùng M-TP", coverUrl: "https://picsum.photos/200?random=11", audioUrl: "", duration: 210 },
  { _id: "s2", id: "s2", title: "Lối Nhỏ", artist: "Đen Vâu ft. Phương Anh Đào", coverUrl: "https://picsum.photos/200?random=12", audioUrl: "", duration: 240 },
  { _id: "s3", id: "s3", title: "Nếu Lúc Đó", artist: "tlinh x 2pillz", coverUrl: "https://picsum.photos/200?random=13", audioUrl: "", duration: 260 },
  { _id: "s4", id: "s4", title: "Waiting For You", artist: "MONO", coverUrl: "https://picsum.photos/200?random=14", audioUrl: "", duration: 195 },
  { _id: "s5", id: "s5", title: "Nấu Ăn Cho Em", artist: "Đen Vâu", coverUrl: "https://picsum.photos/200?random=15", audioUrl: "", duration: 220 }
];
...
const PlaylistAPI = {
  _getPlaylistsFromStorage: () => {
    if (!localStorage.getItem('sp_mock_playlists')) {
      const defaultPlaylists = [
        { _id: "pl1", id: "pl1", name: "Nhạc Lofi Chill Cuối Tuần", songs: _SHARED_SONGS, coverUrl: "https://picsum.photos/200?random=21", description: "Thư giãn đầu óc với những giai điệu lofi nhẹ nhàng mộc mạc." },
        { _id: "pl2", id: "pl2", name: "Coding Focus Mode (Instrumental)", songs: _SHARED_SONGS.slice(0, 3), coverUrl: "https://picsum.photos/200?random=22", description: "Tập trung tối đa hiệu suất làm việc và lập trình." },
        { _id: "pl3", id: "pl3", name: "Giai Điệu Ký Ức V-Pop", songs: _SHARED_SONGS.slice(2, 5), coverUrl: "https://picsum.photos/200?random=23", description: "Sống lại những năm tháng thanh xuân cùng âm nhạc Việt." }
      ];
      localStorage.setItem('sp_mock_playlists', JSON.stringify(defaultPlaylists));
    }
  ...
  create: async (data = {}) => {
    const playlists = PlaylistAPI._getPlaylistsFromStorage();
    const userPlaylistCount = playlists.filter(p => p.name.startsWith("Playlist của tôi")).length;
    
    const newPlaylist = {
      _id: "pl_custom_" + Date.now(),
      id: "pl_custom_" + Date.now(),
      name: data.name || `Playlist của tôi #${userPlaylistCount + 1}`,
      description: data.description || "Playlist do bạn tự tạo bằng chế độ Demo phát triển.",
  ...
  addSong: async (playlistId, songId) => {
    ...
    if (playlist) {
      const targetSong = _SHARED_SONGS.find(s => s._id === songId || s.id === songId);
      // Nếu bài hát tồn tại trong kho và chưa có sẵn trong playlist thì mới thêm vào
      if (targetSong && !playlist.songs.some(s => s._id === songId || s.id === songId)) {
...
const SongAPI = {
  ...
  getById: async (id) => {
    return _SHARED_SONGS.find(s => s._id === id || s.id === id) || _SHARED_SONGS[0];
  },
  search: async (query) => {
    const res = [..._SHARED_SONGS];
    res.data = _SHARED_SONGS;
...
const UserAPI = {
  ...
  changePassword: (currentPassword, newPassword) => apiFetch('/users/password', { method: 'PUT', body: JSON.stringify({ currentPassword, newPassword }) }),
  getLikedSongs: async () => {
    const res = [..._SHARED_SONGS.slice(0, 2)];
    res.data = _SHARED_SONGS.slice(0, 2);
    return res;
  },
  toggleLikeSong: (songId) => apiFetch(`/users/liked-songs/${songId}`, { method: 'POST' }),
};
