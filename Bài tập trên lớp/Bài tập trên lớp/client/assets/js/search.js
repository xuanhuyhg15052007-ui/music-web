...
const Search = (() => {
  ...
  function init() {
    ...
    input.addEventListener('input', (e) => handleInput(e.target.value, dropdown));
  ...
  function showSkeletonResults(dropdown) {
    dropdown.innerHTML = Array(4).fill(0).map(() => `
      <div class="search-result-item">
  ...
  function highlightMatch(text, query) {
    ...
    return text.replace(regex, '<mark style="background:var(--color-accent-subtle);color:var(--color-accent);border-radius:2px">$1</mark>');
  }
